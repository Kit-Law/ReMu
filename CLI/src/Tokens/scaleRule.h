...
namespace ReMu { namespace Tokens {
	...
	{
	public:
		static void evalScaleRule(const Note& initalRootNote, const char* initalScale, const Note& resultRootNote, const char* resultScale, std::string instrument, TransitionTable* transitionTable);
		static void evalScaleRule(const Note& rootNote, const char* scale, std::string instrument, TransitionTable* transitionTable);
